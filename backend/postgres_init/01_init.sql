...
-- Migration 108 (session summary / episode tiering): composite for aged-episode queries
...
-- KG / vector backlogs when Neo4j or Qdrant is unavailable (migrations 152, 153; greenfield merged here)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON kg_write_backlog TO bastion_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON vector_embed_backlog TO bastion_user;
...
-- Migration 108: partial index for conversations flagged for post-idle session summary
...
-- Per-user on-disk cover art cache index (paths relative to MUSIC_COVER_CACHE_DIR)
...
CREATE TABLE IF NOT EXISTS device_tokens (
    ...
    capabilities_policy JSONB NOT NULL DEFAULT '{}'::jsonb
);
...
-- MOBILE PUSH TOKENS + NOTIFICATION AUDIT (Smart Notification Router)
-- Also in migrations/166_notification_router.sql for existing DBs.
...
-- AGENT SKILLS (greenfield: DDL merged from migrations 068, 126, 131, 134, 135, 136, 133)
CREATE TABLE IF NOT EXISTS agent_skills (
    ...
    required_connection_types TEXT[] NOT NULL DEFAULT '{}',
...
ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS is_core BOOLEAN DEFAULT false;
ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS depends_on TEXT[] DEFAULT '{}';
ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS is_candidate BOOLEAN DEFAULT false;
ALTER TABLE agent_skills ADD COLUMN IF NOT EXISTS candidate_weight INTEGER DEFAULT 0;
...
CREATE INDEX IF NOT EXISTS idx_skill_promo_recs_status ON skill_promotion_recommendations(status);
CREATE INDEX IF NOT EXISTS idx_skill_promo_recs_skill ON skill_promotion_recommendations(skill_id);
...
CREATE INDEX IF NOT EXISTS idx_user_home_dashboards_user ON user_home_dashboards(user_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_user_home_dashboard_default
    ON user_home_dashboards(user_id)
    WHERE is_default;

-- Oregon Trail game saves (migration 129; greenfield merged here; user_id matches users.user_id VARCHAR)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON user_home_dashboards TO bastion_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON oregon_trail_saves TO bastion_user;

-- Legacy org-mode GitHub sync tables (removed from schema; idempotent for old volumes)
