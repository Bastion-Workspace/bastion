...
from clients.vector_service_client import (
...
def _deduplicate_chunks(chunks: List[Any]) -> List[Any]:
    """Deduplicate chunks by normalized content hash (same semantics as vector storage)."""
...
class EmbeddingServiceWrapper:
    ...
    async def replay_sync_document_vectors(
        ...
        """Upsert Qdrant points from existing document_chunks (used by backlog drain)."""
        ...
        for ch, emb in zip(unique_chunks, embeddings):
            chunk_to_embedding[_stable_content_hash(ch.content)] = emb
    ...
    async def embed_and_store_chunks(
        ...
        """
        Persist chunks to PostgreSQL first, then embed and upsert to Qdrant.
        On vector/embedding failure, enqueue backlog for later replay.
        """
        if not self._initialized:
            await self.initialize()
        if not chunks:
    ...
    async def _upsert_vectors_from_embeddings(
        ...
        """Upsert Qdrant points only (PostgreSQL chunks must already exist if needed)."""
        try:
    ...
    async def _store_embeddings_with_metadata(
        ...
        """
        Write document_chunks first, then upsert vectors (sidecar / precomputed paths).
        """
        from services import vector_embed_backlog as vb
