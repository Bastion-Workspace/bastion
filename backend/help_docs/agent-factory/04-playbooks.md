...
The same playbook definition is used in all cases. The **agent profile** controls run context (interactive vs background vs scheduled) and where results are sent.
...
4. For **tool** steps, pick the **action** (tool name) and fill required inputs. For **LLM** steps, write the **prompt** and optionally declare output schema. For **LLM agent** steps, list **available_tools** and set **max_iterations**. For **deep agent** steps, define the **phases** list (reason, search, synthesize, evaluate, refine, etc.) and wire next/retry in the phase editor.
