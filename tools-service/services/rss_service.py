...
Now using the centralized DatabaseManager for all database operations!
"""
...
class RSSService:
    ...
    This service handles all RSS database operations and business logic!
    """
    
    def __init__(self):
        # No more connection pool chaos - using centralized DatabaseManager!
        self._database_manager = None
        logger.info("RSS Service initialized with centralized DatabaseManager")
    
    # No more connection pool methods - DatabaseManager handles everything!
    ...
    # Celery pool method removed - DatabaseManager handles all environments!
    
    # RSS Feed Operations
    async def get_feed_by_url(self, feed_url: str, user_id: str) -> Optional[RSSFeed]:
        """Get RSS feed by URL and user ID"""
        try:
            # Using DatabaseManager - no more pool management!
    ...
    async def create_feed(self, feed_data: RSSFeedCreate, update_if_exists: bool = False) -> RSSFeed:
        ...
        try:
            ...
            # Using DatabaseManager for feed creation!
            row_data = await fetch_one("""
            ...
            if row_data:
                ...
                # Note: Immediate feed refresh is handled by backend Celery tasks
                # Tools service doesn't trigger Celery directly
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    ...
                    logger.warning(f"RSS SERVICE: Database connection issue updating feed metadata {feed_id} (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
    ...
    async def get_feed(self, feed_id: str) -> Optional[RSSFeed]:
        ...
        try:
            # Simple, reliable database query - DatabaseManager handles retries!
    ...
    async def get_user_feeds(self, user_id: str, is_admin: bool = False) -> List[RSSFeed]:
        ...
        try:
            # Using DatabaseManager for user feeds!
            if is_admin:
    ...
    async def get_feeds_needing_poll(self, user_id: Optional[str] = None) -> List[RSSFeed]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue getting feeds needing poll (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result == "UPDATE 1":
                    ...
                    logger.warning(f"RSS SERVICE: No rows updated for feed {feed_id}")
                    return False
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue updating feed last_check {feed_id} (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
    ...
    async def mark_feed_polling(self, feed_id: str, is_polling: bool = True) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue marking feed {feed_id} as polling (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
    ...
    async def save_article(self, article: RSSArticle) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue saving article {article.article_id} (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
    ...
    async def get_articles_by_greader_ids(
        ...
        try:
            ...
            articles = [self._row_dict_to_article(dict(r)) for r in rows]
            uniq_requested = set(greader_ids)
            got = {int(a.greader_id) for a in articles if a.greader_id is not None}
            missing = sorted(uniq_requested - got)
            if missing:
                logger.warning(
                    "RSS SERVICE: get_articles_by_greader_ids user=%s requested_unique=%s "
                    "returned_rows=%s missing_count=%s missing_greader_ids_sample=%s",
                    user_id,
                    len(uniq_requested),
                    len(articles),
                    len(missing),
            ...
            logger.info(
                "RSS SERVICE: get_articles_by_greader_ids user=%s requested_ids=%s returned_rows=%s",
                user_id,
                len(greader_ids),
                len(articles),
            )
    ...
    async def get_articles_paginated(
        ...
        *,
        ...
        try:
            ...
            articles = [self._row_dict_to_article(dict(r)) for r in rows]
            null_gid = sum(1 for a in articles if a.greader_id is None)
            logger.info(
                "RSS SERVICE: get_articles_paginated user=%s feed_id=%s starred_only=%s "
                "include_only_read=%s exclude_read=%s older_than_ts=%s limit=%s offset=%s "
                "rows=%s null_greader_id=%s",
                user_id,
                feed_id,
                starred_only,
                include_only_read,
    ...
    async def get_starred_articles(
        ...
        """Articles the user has starred across all feeds, with feed_name from rss_feeds."""
        try:
            sql = """
                ...
                WHERE (a.user_id = $1 OR a.user_id IS NULL)
                  AND a.is_starred IS TRUE
                ORDER BY a.published_date DESC NULLS LAST, a.created_at DESC
                LIMIT $2 OFFSET $3
            """
            rows = await fetch_all(sql, user_id, limit, offset)
            articles = [self._row_dict_to_article(dict(r)) for r in rows]
            logger.info(
                "RSS SERVICE: get_starred_articles user=%s limit=%s offset=%s rows=%s",
                user_id,
                limit,
                offset,
                len(articles),
            ...
            logger.error("RSS SERVICE: get_starred_articles failed: %s", e)
            return []

    async def get_feed_articles_filtered(
        self,
        feed_id: str,
        user_id: str,
        limit: int = 100,
        ...
        """Articles for one feed with optional unread/starred filters (newest first)."""
    ...
    async def search_articles(
        ...
        """Search articles by title/description/full_content. Optional unread/starred filters."""
        ...
        try:
            ...
            pattern_arg = f"%{q}%"
            conds = [
                "(a.user_id = $1 OR a.user_id IS NULL)",
                """(
                    a.title ILIKE $2
            ...
            if unread_only:
                conds.append("(a.is_read IS NOT TRUE)")
            if starred_only:
                conds.append("a.is_starred IS TRUE")
            sql = f"""
                SELECT a.*, f.feed_name AS feed_name
                FROM rss_articles a
                INNER JOIN rss_feeds f ON a.feed_id = f.feed_id
                ...
                LIMIT $3
            """
            params: List[Any] = [user_id, pattern_arg, limit]
            rows = await fetch_all(sql, *params)
            articles = []
            for row_dict in rows:
                rd = dict(row_dict)
                if "images" in rd and isinstance(rd["images"], str):
                    try:
                        rd["images"] = json.loads(rd["images"])
                    except json.JSONDecodeError:
                        rd["images"] = []
                if rd.get("is_starred") is None:
                    rd["is_starred"] = False
                articles.append(RSSArticle(**rd))
            return articles
        except Exception as e:
    ...
    async def is_duplicate_article(self, article: RSSArticle) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue checking duplicate for {article.article_id} (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result == "UPDATE 1":
                    ...
                    logger.warning(f"RSS SERVICE: No rows updated for article {article_id}")
                    return False
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue marking article {article_id} as processed (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
    ...
    async def update_article_full_content(self, article_id: str, full_content: str, images: Optional[List[Dict[str, Any]]] = None, full_content_html: Optional[str] = None) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if any(keyword in error_msg for keyword in ["another operation is in progress", "connection was closed", "connection does not exist"]) and attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 1  # Exponential backoff
                    logger.warning(f"RSS SERVICE: Database connection issue updating full content for article {article_id} (attempt {attempt + 1}/{max_retries}), retrying in {wait_time} seconds: {e}")
                    await asyncio.sleep(wait_time)
                    continue
                else:
    ...
    async def get_articles_for_full_content_backfill(self, limit: int = 10) -> List[RSSArticle]:
        """Articles with no full_content yet; used by scheduled retry (Crawl may have failed on first ingest)."""
        try:
            rows = await fetch_all(
                """
                SELECT * FROM rss_articles
            ...
            logger.error(f"RSS SERVICE ERROR: Failed to get articles for full content backfill: {e}")
            return []
    ...
    async def get_unread_count(self, user_id: str) -> Dict[str, int]:
        ...
        try:
            # Using DatabaseManager for unread count!
            rows = await fetch_all("""
    ...
    async def cleanup_stuck_polling_feeds(self) -> int:
        ...
        try:
            # Using DatabaseManager for cleanup operations!
            result = await execute("""
                UPDATE rss_feeds 
            ...
            if result.startswith("UPDATE "):
                ...
                logger.warning(f"RSS SERVICE: Unexpected result from cleanup: {result}")
                return 0
