...
import { getEnabledModels, getModelRoles } from '../../src/api/models';
...
type FabPhase = 'idle' | 'preparing' | 'recording' | 'transcribing' | 'sending' | 'done' | 'error';
...
export function VoiceFab() {
  ...
  const userChatModelRef = useRef<string>('');
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        if (role) {
          userChatModelRef.current = role;
        } else if (models.length > 0) {
          userChatModelRef.current = models[0].model_id;
        }
      } catch {
        /* keep empty; orchestrator may use server default */
      }
    })();
  }, []);
  ...
  const stopAndSubmit = useCallback(async () => {
    ...
    const model = userChatModelRef.current.trim();
    void streamOrchestrator({
      query: trimmed,
      conversation_id: cid,
      session_id: 'bastion-mobile-voice',
      user_chat_model: model || undefined,
      onChunk: () => {},
    }).catch(() => {
  ...
  const fabBottom = insets.bottom + 56 + 12;
    ...
    <>
      {!modalVisible ? (
        <Pressable
          ...
          accessibilityLabel="Voice note to Bastion"
          ...
          <View style={styles.modalCard}>
            ...
            {phase === 'done' ? (
              <View style={styles.modalBody}>
                ...
                <Text style={styles.modalSub}>Open the Chat tab to read the reply.</Text>
              </View>
            ) : null}
