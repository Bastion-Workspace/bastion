...
- Optional ``section_scope`` on each op: constrain matching and insertion to the
  markdown section that starts at the given heading (through the next heading of
  the same or higher outline level).
...
# ---- Section scope (optional ``section_scope`` on ops) ----

_HEADING_LINE_RE = re.compile(r"^(#{1,6})\s+(.+)$", re.MULTILINE)


def _collect_markdown_headings(content: str, body_start: int, body_end: int) -> List[Dict[str, Any]]:
    """Headings in document order: start offset, full line stripped, level (# count)."""
    headings: List[Dict[str, Any]] = []
    segment = content[body_start:body_end]
    for m in _HEADING_LINE_RE.finditer(segment):
        abs_start = body_start + m.start()
        hashes = m.group(1)
        line = m.group(0).strip()
        headings.append({"start": abs_start, "line": line, "level": len(hashes)})
    return headings


def _pick_section_heading_index(headings: List[Dict[str, Any]], query: str) -> Optional[int]:
    """Resolve query to a heading index: exact, case-insensitive, then substring (prefer longest line)."""
    q = (query or "").strip()
    ...
    for i, h in enumerate(headings):
        hl = h["line"].lower()
        if ql in hl or hl in ql:
            candidates.append((i, len(h["line"])))
    if not candidates:
        return None
...
def _find_section_window(
    ...
    """
    Character range [section_start, section_end) for the section that begins at
    ``section_heading``. ``section_end`` is the start of the next heading whose
    level is <= the section's level (same or higher in the outline), or EOF.

    If the heading cannot be resolved, returns (frontmatter_end, len(content))
    so callers fall back to full-body behavior.
    """
    q = (section_heading or "").strip()
    if not q:
        return frontmatter_end, len(content)
    headings = _collect_markdown_headings(content, frontmatter_end, len(content))
    if not headings:
        logger.warning("section_scope: no markdown headings in document body; ignoring %r", section_heading)
        return frontmatter_end, len(content)
    idx = _pick_section_heading_index(headings, q)
    if idx is None:
        logger.warning("section_scope: heading not found %r; falling back to full body", section_heading)
        return frontmatter_end, len(content)
    start = headings[idx]["start"]
    level = headings[idx]["level"]
    for j in range(idx + 1, len(headings)):
        if headings[j]["level"] <= level:
            return start, headings[j]["start"]
...
def _resolve_replace_delete(
    ...
    if not search_text or not search_text.strip():
        return -1, -1, text, 0.0
    section_scope = (op.get("section_scope") or "").strip()
    if section_scope:
        ws, window_end = _find_section_window(content, section_scope, frontmatter_end)
        window = content[ws:window_end]
    else:
        ws = frontmatter_end
    ...
    end = min(result.end, len(content), window_end)
...
def _find_anchor_text_fuzzy(
    ...
    When ``search_end`` is set, search only within ``content[search_start:search_end]``.
    """
    if not anchor_text:
        return None
    end = len(content) if search_end is None else min(search_end, len(content))
    if search_start >= end:
        return None
    search_content = content[search_start:end]
...
def _resolve_insert(
    ...
    section_scope = (op.get("section_scope") or "").strip()
    if section_scope:
        search_start, section_end = _find_section_window(content, section_scope, frontmatter_end)
    else:
        search_start = frontmatter_end
        section_end = len(content)

    body = content[search_start:section_end].strip()
    if not body:
        if section_scope:
            line_end = content.find("\n", search_start)
            insert_pos = (line_end + 1) if line_end != -1 else search_start
    ...
    result = _find_anchor_text_fuzzy(content, anchor_text, search_start, section_end)
    if not result:
        if section_scope:
            return -1, -1, text, 0.0
        result = _find_anchor_text_fuzzy(content, anchor_text, 0, len(content))
    if not result:
        return -1, -1, text, 0.0
    anchor_pos, confidence = result
    end_pos = _insert_end_pos_for_anchor(content, anchor_pos, anchor_text, op_type, text)
    end_pos = min(end_pos, section_end)
