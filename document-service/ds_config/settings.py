...
class Settings(BaseSettings):
    ...
    # Parallel document processor: resilience (leases, retries, stage timeouts)
    PROCESSING_RESILIENT_WORKER: bool = True
    PROCESSING_MAX_ATTEMPTS: int = 5
    PROCESSING_LEASE_TTL_SECONDS: int = 900
    PROCESSING_LEASE_RENEW_INTERVAL_SECONDS: int = 300
    STAGE_TIMEOUT_PARSE_SECONDS: int = 300
    STAGE_TIMEOUT_EMBED_SECONDS: int = 900
    STAGE_TIMEOUT_KG_SECONDS: int = 120
    RETRY_SCHEDULER_INTERVAL_SECONDS: int = 30
    LEASE_REAPER_INTERVAL_SECONDS: int = 60
    DEP_GUARD_FAILURE_THRESHOLD: int = 5
    DEP_GUARD_COOLDOWN_SECONDS: int = 60
    ...
    # Document-service container runtime (gRPC server + NER)
    ...
    # Parallel document pipeline (document-service only)
    DS_MAX_INFLIGHT_POST_UPLOAD_TASKS: int = Field(
        default=24,
        ge=1,
        description="Max concurrent post-upload processing tasks per process (backpressure).",
    )
    DS_MAX_CONCURRENT_DOCUMENTS: Optional[int] = Field(
        default=None,
        description="Override ParallelDocumentProcessor max concurrent documents; unset keeps built-in default.",
    )
    DS_THREAD_POOL_SIZE: Optional[int] = Field(
        default=None,
        description="Override hybrid/thread PDF-DOCX-EPUB extraction pool size; unset keeps built-in default.",
    )

    @field_validator("DS_MAX_CONCURRENT_DOCUMENTS", "DS_THREAD_POOL_SIZE", mode="before")
    @classmethod
    def _optional_int_empty_to_none(cls, v):
        if v is None or v == "":
            return None
