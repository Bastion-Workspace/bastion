...
from ds_models.api_models import DocumentInfo, ProcessingStatus
from ds_services.folder_service import FolderService
from shims.utils.websocket_manager import get_websocket_manager
...
class FileManagerService:
    ...
    async def initialize(self):
        ...
        if service_container.is_initialized:
            ...
            from ds_processing.parallel_document_processor import ProcessingConfig, ProcessingStrategy
    ...
    async def place_file(self, request: FilePlacementRequest) -> FilePlacementResponse:
        ...
        try:
            ...
            try:
                ...
                from ds_models.api_models import DocumentType
            ...
            if folder_id:
                try:
                    # Resolve folder with caller identity so user-owned folders are visible
                    folder_details = await self.folder_service.get_folder(
                        folder_id,
                        request.user_id,
                        getattr(request, "current_user_role", None) or "user",
                    )
                    if folder_details:
                        ...
                        if folder_collection_type:
                            ...
                            elif folder_collection_type == 'team':
                                # DB CHECK: team rows require user_id NULL and team_id NOT NULL
                                final_user_id = None
                                tid = getattr(folder_details, "team_id", None)
                                final_team_id = str(tid) if tid is not None else None
                                logger.info(
                                    "📋 COLLECTION TYPE INHERITANCE: Folder %s is TEAM, document team_id=%s",
                                    folder_id,
                                    final_team_id,
                                )
                            else:
            ...
            if final_collection_type == "team" and not final_team_id:
                raise ValueError(
                    "Team folder is missing team_id; cannot create document_metadata row for collection_type=team"
                )
            ...
            if getattr(request, "content_bytes", None):
                ...
                markdown_file_path = await self._save_as_markdown_file(
                    ...
                    folder_id,
                    document_metadata,
                    disk_user_id=final_user_id,
                    disk_collection_type=final_collection_type,
                    disk_team_id=final_team_id,
                )
    ...
    async def _save_as_markdown_file(
        ...
        *,
        disk_user_id: Optional[str] = None,
        disk_collection_type: Optional[str] = None,
        disk_team_id: Optional[str] = None,
    ) -> str:
        """
        ...
        """
        try:
            from ds_services.markdown_formatter_service import get_markdown_formatter
            from pathlib import Path
            from ds_config import settings
            ...
            if request.source_type == SourceType.MANUAL:
                ...
                path_uid = disk_user_id if disk_user_id is not None else request.user_id
                path_ct = disk_collection_type or request.collection_type
                path_tid = disk_team_id
                file_path_str = await self.folder_service.get_document_file_path(
                    filename=filename,
                    folder_id=folder_id,
                    user_id=path_uid,
                    collection_type=path_ct,
                    team_id=path_tid,
                )
                file_path = Path(file_path_str)
