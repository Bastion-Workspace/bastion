...
class FolderService:
    ...
    async def get_document_file_path(
        ...
        *,
        ...
        Default ensure_directory=True: document-service owns the upload volume and
        may create parent dirs before writes. Set False for read-only resolution.
    ...
    async def get_folder_tree(
        ...
        try:
            ...
            if user_id:
                try:
                    from shims.services.team_service import TeamService
    ...
    async def remove_team_upload_directory(self, team_id: str) -> bool:
        """Remove the entire upload tree for a team (Teams/<team_id>/...) on this host."""
        try:
            import shutil

            root = self.teams_base / str(team_id)
            if root.exists():
                shutil.rmtree(root)
                logger.info("Removed team upload directory: %s", root)
            return True
        except Exception as e:
            logger.error("Failed to remove team upload directory for %s: %s", team_id, e)
            return False
    ...
    async def delete_folder(
        ...
        *,
        allow_team_root: bool = False,
    ) -> bool:
        """Delete a folder: all documents under it (full delete_document), then folder row(s), then disk.

        If recursive is false and the folder has any descendant folders or documents, raises ValueError.
        Team root folders require allow_team_root=True (team deletion flow only).
        """
        try:
            folder = await self.get_folder(folder_id, user_id, current_user_role)
            if not folder:
            ...
            if (
                folder.collection_type == "team"
                and folder.parent_folder_id is None
                and not allow_team_root
            ):
                raise ValueError(
                    "Cannot delete team root folder from the library UI. "
                    "Delete the team instead, or use the team deletion flow."
                )
            ...
            folder_path = await self.get_folder_physical_path(
                folder_id, user_id=query_user_id, user_role=query_user_role
            )
            logger.info("Physical path for folder %s: %s", folder_id, folder_path)

            descendant_folder_ids, descendant_document_ids = (
                await self.document_repository.get_folder_descendants(folder_id, user_id)
            )
            has_contents = bool(descendant_folder_ids) or bool(descendant_document_ids)
            if not recursive and has_contents:
                raise ValueError(
                    "Folder is not empty (contains subfolders and/or documents). "
                    "Pass recursive=true to delete the folder and all contents."
                )
            ...
            for doc_id in descendant_document_ids:
                ...
                if not ok:
                    raise RuntimeError(
                        f"Failed to delete document {doc_id} while removing folder {folder_id}; "
            ...
            if not deletion_success:
                logger.error(
                    "Permission denied: Cannot delete folder %s (RLS policy blocked)",
                    folder_id,
                )
                raise PermissionError(
                    "Permission denied: You do not have permission to delete this folder"
                )
            ...
            if folder_path and folder_path.exists():
                try:
                    ...
                    logger.info("Deleted physical directory: %s", folder_path)
                except Exception as e:
                    logger.error("Failed to delete physical directory %s: %s", folder_path, e)
            else:
                logger.warning("Physical directory not found or path unavailable: %s", folder_path)
