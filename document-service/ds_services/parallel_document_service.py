...
from ds_processing.parallel_document_processor import (
    ParallelDocumentProcessor, ProcessingConfig, ProcessingStrategy
)
from ds_services.document_service_v2 import DocumentService
from ds_services.zip_processor_service import ZipProcessorService
...
class ParallelDocumentService(DocumentService):
    ...
    def __init__(self):
        ...
        self._post_upload_processing_semaphore: Optional[asyncio.Semaphore] = None
    ...
    async def _schedule_post_upload_processing(
        ...
        """Acquire backpressure slot before spawning work; release when processing finishes."""
        if self._post_upload_processing_semaphore is None:
            await super()._schedule_post_upload_processing(
                document_id, file_path, doc_type, user_id
            )
            return

        await self._post_upload_processing_semaphore.acquire()

        async def _run() -> None:
            try:
                await self._process_document_async(document_id, file_path, doc_type, user_id)
            finally:
                self._post_upload_processing_semaphore.release()
    ...
    async def initialize(
        ...
        limit = max(1, int(getattr(settings, "DS_MAX_INFLIGHT_POST_UPLOAD_TASKS", 24)))
        self._post_upload_processing_semaphore = asyncio.Semaphore(limit)
        
        # Initialize base service with shared dependencies
        await super().initialize(
            shared_document_repository,
            shared_embedding_manager,
            shared_kg_service,
        ...
        if enable_parallel:
            if processing_config is not None:
                ...
                if settings.DS_MAX_CONCURRENT_DOCUMENTS is not None:
                    cfg = replace(cfg, max_concurrent_documents=settings.DS_MAX_CONCURRENT_DOCUMENTS)
                if settings.DS_THREAD_POOL_SIZE is not None:
                    cfg = replace(cfg, thread_pool_size=settings.DS_THREAD_POOL_SIZE)
            self.parallel_processor = ParallelDocumentProcessor(cfg)
    ...
    async def upload_and_process(
        ...
        team_id: str = None,
        folder_service=None,
        exempt_from_vectorization_override=None,
        ...
        try:
            ...
            if doc_type == 'zip':
                ...
                result = await super().upload_and_process(
                    file,
                    doc_type,
                    user_id,
                    folder_id,
                    ...
                    exempt_from_vectorization_override=exempt_from_vectorization_override,
                )
        ...
        try:
            ...
            if self.parallel_processor:
                ...
                if parallel_status["status"] != "not_found":
                    base_status.progress = float(parallel_status.get("progress", base_status.progress))
                    base_status.processing_stage = parallel_status.get("processing_stage")
                    base_status.attempt_count = int(parallel_status.get("attempt_count") or 0)
                    base_status.last_error_kind = parallel_status.get("last_error_kind")
                    na = parallel_status.get("next_attempt_at")
                    if na:
                        try:
                            base_status.next_attempt_at = datetime.fromisoformat(
                                na.replace("Z", "+00:00")
                            )
                        except Exception:
                            base_status.next_attempt_at = None
                    st = parallel_status["status"]
                    if st == "processing":
                        base_status.message = (
                            f"Processing (queue hint: {parallel_status.get('queue_position', 0)}); "
                            f"stage={parallel_status.get('processing_stage') or 'n/a'}"
                        )
                    elif st == "completed":
                        ...
                        err = parallel_status.get("last_error") or parallel_status.get("error")
                        base_status.message = f"Parallel processing failed: {err or 'Unknown error'}"
                    elif st == "retry_scheduled":
                        base_status.message = (
                            f"Retry scheduled (attempt {base_status.attempt_count}); "
                            f"next at {parallel_status.get('next_attempt_at') or 'n/a'}"
                        )
                ...
                async with recovery_semaphore:
                    try:
                        if getattr(settings, "PROCESSING_RESILIENT_WORKER", True):
                            lu = getattr(doc_info, "locked_until", None)
                            if lu is not None:
                                ...
                                if lu > now:
                                    logger.info(
                                        "Skipping resume for %s: active processing lease",
                ...
                try:
                    if getattr(settings, "PROCESSING_RESILIENT_WORKER", True):
                        ...
                        if lu is not None:
                            ...
                            if lu > now:
                                logger.info(
                                    "Skipping sequential resume for %s: active processing lease",
                                    doc_info.document_id,
                                )
                                continue
                    ...
                    if file_path and file_path.exists():
                        ...
                        await self._schedule_post_upload_processing(
                            doc_info.document_id,
                            file_path,
                            doc_type,
                            getattr(doc_info, "user_id", None),
                        )
