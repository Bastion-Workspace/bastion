...
def _fallback_append_proposed_text_if_unresolved(content: str, raw_op: Dict[str, Any]) -> Optional[str]:
    ...
    """
    from ds_utils.editor_operations_resolver import normalize_insertion_spacing
...
async def update_document_metadata_tool(
    ...
    **SECURITY**: Only updates user's own documents (collection_type='user')
    ...
    try:
        ...
        from ds_utils.frontmatter_utils import parse_frontmatter, build_frontmatter
...
async def update_document_content_tool(
    ...
    user_id: str = "system",
    append: bool = False,
    write_initiator: str = "agent_tool",
) -> Dict[str, Any]:
    """
    ...
    **SECURITY**: Only updates user's own documents (collection_type='user')
    ...
    Args:
        ...
        write_initiator: "user_api" for REST/editor saves (versioning + WS, no agent toast);
            default "agent_tool" for orchestrator/tool writes.
    
    Returns:
        Dict with success, document_id, content_length, and message
    """
    try:
        is_user_api = (write_initiator or "").strip() == "user_api"
        version_change_source = "user_save" if is_user_api else "agent_edit"
        completed_ws_source = "embedding" if is_user_api else "agent"
        ...
        # Security: user-owned docs must match user_id; global/team updates rely on caller auth.
        doc_user_id = getattr(doc_info, 'user_id', None)
        doc_collection_type = getattr(doc_info, 'collection_type', 'user')
        ...
        if doc_collection_type == "user":
            if doc_user_id and doc_user_id != user_id:
                return {
                    ...
                    "error": "Access denied",
                    "message": (
                        f"Document belongs to different user (document user: {doc_user_id}, "
        ...
        if append:
            ...
            # Editor saves (user_api) send the full markdown file; must not strip/re-merge YAML or
            # frontmatter edits are lost (old FM dict + new body only).
            if is_user_api:
                new_content = content
                logger.info(
                    f"Replacing entire content ({len(current_content)} chars) with user payload "
                    f"({len(new_content)} chars, verbatim)"
                )
            elif content.strip().startswith("---"):
                # Agent sent a file that opens with YAML fence — treat as whole-document replace
                new_content = content
                logger.info(
                ...
                # Body-only replace: keep on-disk frontmatter, replace body
        ...
        try:
            from ds_services.document_version_service import snapshot_before_write
            await snapshot_before_write(document_id, user_id, version_change_source, None, None)
        except Exception as verr:
            logger.warning("Version snapshot before write failed (non-fatal): %s", verr)
        ...
        if is_exempt:
            ...
            await document_service._emit_document_status_update(
                document_id, ProcessingStatus.COMPLETED.value, user_id, content_source=completed_ws_source
            )
            return {
                "success": True,
                "document_id": document_id,
...
async def propose_document_edit_tool(
    ...
    **SECURITY**: Only proposes edits for user's own documents (collection_type='user')
    ...
    try:
        ...
        from ds_db.database_manager.database_helpers import execute, fetch_value
    ...
    **SECURITY**: Only allowed for specific trusted agent names (see ALLOWED_AGENTS),
    ...
    try:
        ...
        if playbook_auto_apply:
            from ds_utils.editor_operations_resolver import resolve_operations
