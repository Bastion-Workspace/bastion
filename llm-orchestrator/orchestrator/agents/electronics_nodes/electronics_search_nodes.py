"""
Electronics Agent - Search Nodes Module
...
class ElectronicsSearchNodes:
    """Search-related nodes for electronics agent"""
    ...
    async def analyze_information_needs_node(self, state) -> Dict[str, Any]:
        ...
        - What components/protocols/architecture are relevant
        ...
        try:
            ...
            if frontmatter:
                project_components = frontmatter.get("components", [])
                project_protocols = frontmatter.get("protocols", [])
                project_architecture = frontmatter.get("architecture", [])
            
            # Build project context dict for tool
            project_context = {
                "components": project_components,
                "protocols": project_protocols,
                "architecture": project_architecture
            }
            
            # Extract components from referenced files
            if referenced_context:
                for category, files in referenced_context.items():
                    if category == "components" and isinstance(files, list):
                        for file_doc in files:
                            if isinstance(file_doc, dict):
                                filename = file_doc.get("filename", "")
                                if filename and filename not in project_context.get("components", []):
                                    project_context.setdefault("components", []).append(filename)
            ...
            # Map generic "relevant_entities" back to electronics-specific fields for backward compatibility
            relevant_entities = info_result.get("relevant_entities", [])
            # Split entities into components and protocols (heuristic)
            relevant_components = []
            relevant_protocols = []
            for entity in relevant_entities:
                # Simple heuristic: if it's a known protocol, add to protocols
                if any(proto.lower() in entity.lower() for proto in ["i2c", "spi", "uart", "usb", "can", "ethernet", "wifi", "bluetooth"]):
                    relevant_protocols.append(entity)
                else:
                    relevant_components.append(entity)
            
            # Merge with existing project context
            result_dict = dict(info_result)
            result_dict["relevant_components"] = list(set(relevant_components + project_context.get("components", [])))
            result_dict["relevant_protocols"] = list(set(relevant_protocols + project_context.get("protocols", [])))
    ...
    async def generate_project_aware_queries_node(self, state) -> Dict[str, Any]:
        """
        Generate project-aware search queries. If search_queries already set by analyze node (via enhance_query_tool), pass through.
        """
        try:
            ...
            project_components = information_needs.get("relevant_components", [])
            project_protocols = information_needs.get("relevant_protocols", [])
            
            if not project_components and frontmatter:
                project_components = frontmatter.get("components", [])[:5]
            ...
            domain_examples = [
                ...
                "ADW221S relay driver circuit specifications"
            ]
    ...
    async def search_content_node(self, state) -> Dict[str, Any]:
        """
        Node: Semantic segment search for electronics content.
        ...
        try:
            ...
            # Electronics-specific domain keywords for content extraction
            electronics_keywords = [
                'circuit', 'schematic', 'voltage', 'current', 'resistance', 'capacitance',
                'microcontroller', 'sensor', 'motor', 'led', 'relay', 'transistor',
                'power', 'supply', 'battery', 'solar', 'usb', 'i2c', 'spi', 'uart',
                'analog', 'digital', 'pwm', 'adc', 'dac', 'interrupt', 'timer',
                'arduino', 'esp32', 'esp8266', 'raspberry', 'stm32'
            ]
            ...
            return {
                "documents": documents,
                "segments": segments_list,  # Keep segments separate for quality assessment
    ...
    async def assess_result_quality_node(self, state) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check for verification/checking keywords that indicate web search is needed
            query_lower = query.lower()
            ...
            result_summary = []
            for i, seg in enumerate(segments[:10], 1):  # Top 10 segments
...
- 0.5-0.7: Results partially answer, RECOMMEND web search for verification/latest specs/details
...
{{
  ...
  "missing_information": ["what's missing if any"]
            ...
            try:
                ...
                if len(segments) == 0:
                    ...
                    result_dict = {
                        ...
                        "adequately_answers_query": True,
                        "relevant_to_project": False,
                        "needs_web_search": query_type in ["component_selection"],  # Component specs often need web
                        "should_re_search": False,
                        "reasoning": "Found library documents but may need latest specs",
                        "missing_information": []
                    }
            ...
            # Also trigger web search for moderate quality scores (0.5-0.7) when verification is requested
            ...
            # Default: assume moderate quality, try web search
            return {
