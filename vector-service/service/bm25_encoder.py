...
def get_default_bm25_encoder() -> BM25Encoder:
    """Lazy singleton: load shipped IDF JSON or fall back to synthetic English IDF."""
    ...
    try:
        enc = BM25Encoder(k1=float(os.environ.get("BM25_K1", "1.5")), b=float(os.environ.get("BM25_B", "0.75")))
        enc.load(idf_path)
        _default_encoder = enc
        return _default_encoder
    except FileNotFoundError:
        logger.warning("BM25 default IDF file not found at %s — using synthetic English IDF", idf_path)
    except Exception as exc:
        logger.warning(
            "Failed to load BM25 IDF from %s: %s — using synthetic English IDF",
            idf_path,
            exc,
        )
